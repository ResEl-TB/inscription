...
def get_free_alias(uid):
    ...
    if (search("ou=machines,dc=resel,dc=enst-bretagne,dc=fr", "(Hostalias={})".format(test)) != None) or (search("ou=machines,dc=resel,dc=enst-bretagne,dc=fr", "(Host={})".format(test)) != None):
        continuer = True
        i = 2
        while continuer:
